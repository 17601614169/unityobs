...
static void AddExtraModulePaths()
{
	...
	int ret = GetProgramDataPath(base_module_dir, sizeof(base_module_dir),
				     "yl-custom-service/plugins/%module%");
#else
	...
	BPtr<char> config_bin =
		os_get_config_path_ptr("yl-custom-service/plugins/%module%/bin");
	BPtr<char> config_data =
		os_get_config_path_ptr("yl-custom-service/plugins/%module%/data");
...
static void select_leftwidget_item(obs_sceneitem_t *item, bool select) {
	OBSBasic *main = reinterpret_cast<OBSBasic *>(App()->GetMainWindow());
	if (main) {
		main->SelectLeftWidgetItem(item, select);
	}
}
...
OBSBasic::OBSBasic(QWidget *parent)
	...
	,m_pBottomCtrl(nullptr)
	,m_pLeftWidget(nullptr)
	,m_pMicSlider(nullptr)
	,m_pAudioSlider(nullptr)
	,m_pRecordSaveTip(nullptr)
	,properties(nullptr)
	,m_pUserInfoWidget(nullptr)
	//,m_pStompClient(nullptr)
	,m_pSettingDialog(nullptr)
	,m_pIMWidget(nullptr)
	,m_pSacrougeDialog(nullptr)
	,m_pRedDetailsDialog(nullptr)
	,trayIcon(nullptr)
	,m_pProcess(nullptr)
	,m_pChatWebWidget(nullptr)
	,m_pUrlTabWidget(nullptr)
	,m_pStackedLayout(nullptr)
	,m_bEnterRoom(false)
	,m_pRoomListWebWidget(nullptr)
	,m_pExchangeWebWidget(nullptr)
	,m_pRedPacketWebWidget(nullptr)
	,m_pWebData(nullptr)
	,m_pWebDetail(nullptr)
	,m_pKnowledgeWebWidget(nullptr)
	,m_pQuickReplyWebWidget(nullptr)
	,m_pCheckHeartbeat(nullptr)
	,m_pCheckInHeartbeat(nullptr)
	,m_curSelectItem(nullptr)
	,m_pStreamTimer(nullptr)
	,m_bStreamOperate(false),
	  m_pInvalidDlg(nullptr)
{
	blog(LOG_INFO,"obs basic");
	...
	ui->setupUi(this);
	QString strBorder = QString::fromUtf8("QMainWindow#obsborder{border:1px solid black;}");
	this->setStyleSheet(strBorder);
	this->setObjectName("obsborder");
	
	QPixmap pixmap = common::GetPixmap(PIXMAP_TRAY_ICON);
	QIcon icon(pixmap);
	setWindowIcon(icon);
	...
	//add by huchenzhi
	InitCustomCtrls();
	blog(LOG_INFO, "init custom");
	
	//获取当前是否为最大化
	bool bMax = DataConfig::GetInstance()->GetMaximum();
	if (m_pTopWidget)
		m_pTopWidget->SetMax(false);
	m_bStartStream = false;
	InitStompClient();
	InitConnections();
	GetLiveList();
	GetGiftList();
	m_pCheckHeartbeat = new QTimer(this);
	m_pCheckHeartbeat->setInterval(6000);
	m_pTimerPreviewPos= new QTimer(this);
	m_pTimerPreviewPos->setInterval(500);
	auto PosSlot = [&]() {
		ui->preview->move(0,0);
	};
	connect(m_pTimerPreviewPos, &QTimer::timeout, PosSlot);
	m_pTimerPreviewPos->start();

	auto HeartSlot = [&]() {
		CheckHearBeat();
	};
	connect(m_pCheckHeartbeat, &QTimer::timeout, HeartSlot);
	m_pCheckHeartbeat->start();
	blog(LOG_INFO,"obs basic ok");

	m_pStreamTimer= new QTimer(this);
	m_pStreamTimer->setInterval(2500);
	auto StreamTimerSlot = [&]() {
		m_bStreamOperate = false;
		m_pStreamTimer->stop();
	};
	connect(m_pStreamTimer, &QTimer::timeout, StreamTimerSlot);

	m_pCheckInHeartbeat = new QTimer(this);
	m_pCheckInHeartbeat->setInterval(60000);
	auto CheckInHeartSlot = [&]() {
		CheckSignInHearBeat();
	};
	connect(m_pCheckInHeartbeat, &QTimer::timeout, CheckInHeartSlot);
}
...
retryScene:
	...
	if (api) {
		...
		if (m_pLeftWidget){
			for (auto item : items) {
				m_pLeftWidget->AddSourceItem(item);
			}
		}
	}
}
...
bool OBSBasic::LoadService()
{
	...
	obs_data_set_default_string(data, "type", "webrtc_mediasoup");
...
bool OBSBasic::InitService()
{
	...
	service = obs_service_create("webrtc_mediasoup", "default_service", nullptr,
				     nullptr);
	if (!service)
		return false;
	...
	if (!data) {
		OBSData data2 = obs_data_create();

		obs_data_set_default_string(data2, "type", "webrtc_mediasoup");
		type = obs_data_get_string(data2, "type");
		obs_data_t *settings = obs_data_get_obj(data2, "settings");
		obs_data_t *hotkey_data = obs_data_get_obj(data2, "hotkeys");
	} else {
		obs_data_set_default_string(data, "type", "webrtc_mediasoup");
		type = obs_data_get_string(data, "type");
		obs_data_t *settings = obs_data_get_obj(data, "settings");
	...
	OBSData url_setting = obs_data_create();
	obs_data_release(url_setting);
	obs_data_set_string(url_setting, "server", MEDIASOUP_URL.c_str());
	OBSService newService = obs_service_create(
		"webrtc_mediasoup", "default_service",
						   url_setting, nullptr);
...
bool OBSBasic::InitBasicConfig()
{
	...
	QString strShow = QT_UTF8(configPath);
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, 512, "yl-custom-service/basic/scenes/%s.json",
		       sceneCollection);
	if (ret <= 0)
		throw "Failed to create scene collection file name";

	ret = GetConfigPath(savePath, sizeof(savePath), fileName);
	QString strDisplay = QT_UTF8(fileName);
	if (ret <= 0)
...
#endif
#if _DEBUG
	SetOriginCtrlsVisible(true);
#else
	SetOriginCtrlsVisible(false);
#endif
	GetIMToken();
//#if	_DEBUG
...
bool OBSBasic::CreateSecretWindow(obs_source_t *source)
{
	bool set = false;
	bool closed = true;
	if (properties)
	...
	properties = new OBSBasicProperties(this, source);
	set = properties->SetSecretPath();
	properties->deleteLater();
	properties = nullptr;
	return set;
}

bool OBSBasic::CreateCameraWindow(obs_source_t *source)
{
	bool set = false;
	bool closed = true;
	if (properties)
...
static bool select_one(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	...
	obs_sceneitem_select_left_widget(item, (selectedItem == item), select_leftwidget_item);
...
void OBSBasic::AddSceneItem(OBSSceneItem item)
{
	...
	if (GetCurrentScene() == scene) {
		ui->sources->Add(item);
		if (m_pLeftWidget){
			m_pLeftWidget->AddSourceItem(item, true);
		}
	}
...
void OBSBasic::ActivateAudioSource(OBSSource source)
{
	...
	//add by huchenzhi
	InitMicAudioId();
	const char *deviceId = "default"; //设置默认音频设备
	...
	const char *id = obs_source_get_id(source);
	if (!strcmp(id, "wasapi_input_capture") && !m_strMicId.empty()
		&& deviceId && !strcmp(m_strMicId.c_str(), deviceId)) {
		connect(m_pMicSlider, SIGNAL(valueChanged(int)), vol,
			SLOT(SliderChanged(int)));
		blog(LOG_INFO, "mic wasapi_output_capture ");
	}
	else if (!strcmp(id, "wasapi_output_capture") && !m_strAudioId.empty()
		&& deviceId &&!strcmp(m_strAudioId.c_str(), deviceId)) {
		connect(m_pAudioSlider, SIGNAL(valueChanged(int)), vol,
			SLOT(SliderChanged(int)));
		blog(LOG_INFO, "audio wasapi_output_capture ");
	}
}
...
void OBSBasic::RemoveSelectedSceneItem()
{
	...
	if (item) {
		obs_source_t *source = obs_sceneitem_get_source(item);
		if (QueryRemoveSource(source))
			obs_sceneitem_remove_left_widget(
				item, remove_leftwidget_item);
	}
}
...
void OBSBasic::ReorderSources(OBSScene scene)
{
	...
	if (m_pLeftWidget) {
		...
		OBSScene scene = GetCurrentScene();
		obs_scene_enum_items(scene, EnumItem, &items);
		m_pLeftWidget->ReorderItems(items);
	}
...
void OBSBasic::closeEvent(QCloseEvent *event)
{
	...
	if (isVisible()) {
		...
		// add by cjf
		DataConfig::GetInstance()->SetMaximum((this->windowState() == Qt::WindowMaximized) ? true : false);
		// add by cjf
	}

	if (outputHandler && outputHandler->Active()) {
		SetShowing(true);
		MSGBTN button = MsgBox::information(
			this, QTStr("ConfirmExit.Title"),
			QTStr("ConfirmExit.Text"),EOK_CANCLE);

		if (button == ECANCLE) {
			event->ignore();
			restart = false;
			return;
		} else if (m_bStartStream) {
			ShowEndLiveInfo();
...
{
	if (event->type()==UserMoveSceneItemType){
		UserMoveSceneItemEvent *eventSceneItem =
			dynamic_cast<UserMoveSceneItemEvent*>(event);
		if (eventSceneItem!=nullptr){
			std::string closeName = StringConvert::GetInstance()->GetString(STR_CLOSE_SOURCE).toStdString();
			do {
				if (eventSceneItem->hide) {
					obs_source_t *source = nullptr;
					source = obs_get_source_by_name(closeName.c_str());
					if (source==nullptr)
						break;
					obs_sceneitem_t *item2 = obs_scene_find_source(GetCurrentScene(), closeName.c_str());
					if (item2) {
						obs_sceneitem_set_visible(item2,false);
					}
				}else{
					if (!closeName.empty() && strlen(eventSceneItem->name.c_str())>0) {
					if (eventSceneItem->name.compare(closeName)==0)
						break;
					obs_source_t *source = nullptr;
					source = obs_get_source_by_name(closeName.c_str());
					if (source==nullptr)
					...
					if (item2) {
						vec2 pos;
						pos.x=eventSceneItem->x -30;
						pos.y=eventSceneItem->y -30;
						obs_sceneitem_set_pos(item2, &pos);
						if (eventSceneItem->id.compare("image_source")==0 ||
							eventSceneItem->id.compare("text_gdiplus_v2")==0){
							obs_sceneitem_set_visible(item2,true);
							obs_sceneitem_set_order(item2,OBS_ORDER_MOVE_TOP);
						}else{
							obs_sceneitem_set_visible(item2,false);
						}
					}
...
bool OBSBasic::nativeEvent(const QByteArray &eventType, void *message, long *result){
	...
	return OBSMainWindow::nativeEvent(eventType,message,result);
}
...
{
#if CUSTOM_POP_ENABLED
	OBSSceneItem sceneItem = ui->sources->Get(idx);
	obs_source_t *source = obs_sceneitem_get_source(sceneItem);
	if (source) {
		const char * source_name = obs_source_get_name(source);
		std::string secret_name = StringConvert::GetInstance()->GetString(STR_SECRET).toStdString();
		if (stricmp(source_name, m_strCloseName.c_str()) != 0 && stricmp(source_name, secret_name.c_str()) != 0) {
			QMenu popup(this);
			popup.setStyleSheet("QMenu {border:1px solid #D3CCDF; background-color:rgb(255,255,255);}"
			...
			popup.addAction(QTStr("Basic.MainMenu.ResetSize"),this,SLOT(on_actionResetTransform_triggered()));

			const char *id = obs_source_get_id(source);
			if (stricmp(id, "text_gdiplus") == 0 || stricmp(id, "text_gdiplus_v2") == 0) {
				popup.addAction(QTStr("Basic.MainMenu.EditText"), this, SLOT(on_actionSourceProperties_triggered()));
			} else if (stricmp(id, "image_source") == 0) {
				popup.addAction(QTStr("Basic.MainMenu.StretchToScreen"),this, SLOT(on_actionStretchToScreen_triggered()));
			}
			popup.addAction(QTStr("Basic.MainMenu.Delete"),this, SLOT(on_actionRemoveSource_triggered()));
			popup.exec(QCursor::pos());
		}
	}
...
void OBSBasic::AddSource(const char *id)
{
	...
	if (id && *id) {
		if (stricmp(id, "window_capture") == 0) {
			WindowCaptureDialog dialog(GetCurrentScene(), id, this);
			dialog.exec();
		} else if (stricmp(id, "dshow_input") == 0) {
			CameraDialog dialog(GetCurrentScene(), id, this);
			dialog.exec();
		}
		else if (stricmp(id, "monitor_capture") == 0) {
			auto threadfunc = [&] {
				QString strExe("");
				#if defined(QT_DEBUG)
					strExe = "QCaptureScreend.exe";
				#   else
					strExe = "QCaptureScreen.exe";
				#  endif
				...
				if (file.exists()) {
					...
					connect(m_pProcess, &QProcess::readyReadStandardOutput, this, &OBSBasic::OnReadStandardOutput);
					m_pProcess->start(strExe, QIODevice::ReadOnly);
					m_pProcess->waitForStarted();
					m_pProcess->waitForFinished();
				}
			};
			QThread *thread = QThread::create(threadfunc);
			thread->start();
			thread->wait();
			thread->deleteLater();
		}else if (stricmp(id, "text_gdiplus") == 0) {
			FontSet* set = new FontSet(nullptr,this);
			connect(set,&FontSet::addTxtSignal,this,&OBSBasic::OnAddFontTxt);
			set->setAttribute(Qt::WA_DeleteOnClose);
			set->raise();
			set->show();
		}else if (stricmp(id, "image_source") == 0) {
			PictureDialog dialog(GetCurrentScene(), id, this);
			dialog.exec();
		} 
		else {
	...
	if (!strName.empty()) {
		OBSScene scene = GetCurrentScene();
		obs_sceneitem_t *item = obs_scene_find_source(scene, strName.c_str());
		if (item) {
			obs_sceneitem_set_order(item, OBS_ORDER_MOVE_TOP);
			StretchToScreen(item);
		}
	}
}
...
void OBSBasic::on_actionRemoveSource_triggered()
{
	...
	if (items.size() == 1) {
		...
		obs_source_t *source = obs_sceneitem_get_source(item);
		const char *name = obs_source_get_name(source);
		if (stricmp(name, m_strCloseName.c_str()) == 0)
			return;

		if (source /*&& QueryRemoveSource(source)*/) {
			obs_sceneitem_remove_left_widget(
				item, remove_leftwidget_item);
			UserMoveSceneItemEvent* event = new UserMoveSceneItemEvent;
			event->hide = true;
...
void OBSBasic::on_actionSourceProperties_triggered()
{
	...
	if (source) {
		const char *id = obs_source_get_id(source);
		if (stricmp(id, "text_gdiplus") == 0 ||
		    stricmp(id, "text_gdiplus_v2") == 0) {
			FontSet *set = new FontSet(source, this);
			connect(set, &FontSet::addTxtSignal, this,
				&OBSBasic::OnAddFontTxt);
			set->setAttribute(Qt::WA_DeleteOnClose);
			set->raise();
			set->show();
		} else {
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "yl-custom-service/logs") <= 0)
		return;
...
{
	UploadLog("yl-custom-service/logs", App()->GetCurrentLog(), false);
}

void OBSBasic::on_actionUploadLastLog_triggered()
{
	UploadLog("yl-custom-service/logs", App()->GetLastLog(), false);
}
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "yl-custom-service/crashes") <= 0)
		return;
...
{
	UploadLog("yl-custom-service/crashes", App()->GetLastCrashLog(), true);
}
...
{
	outputHandler->ResetReconnect(false);
...
inline void OBSBasic::OnDeactivate()
{
	if (!outputHandler->Active() && !ui->profileMenu->isEnabled()) {
		...
		if (trayIcon && trayIcon->isVisible())
			trayIcon->setIcon(QIcon::fromTheme(
				"obs-tray", QIcon(":/res/images/keepfun.ico")));
...
void OBSBasic::StreamingStart()
{
	...
	//add by huchenzhi
	SetLiveBtnText(STR_STOP_LIVE, true);
	StreamStateSignal(STREAM_START);
}
...
void OBSBasic::StreamingStop(int code, QString last_error)
{
	...
	case OBS_OUTPUT_ERROR:
		...
		break;
	case OBS_OUTPUT_WEBSOCKET_SHUTDOWN:
		errorDescription = Str("Output.WebsocketShutdown.Error");
		break;
	case OBS_OUTPUT_MEDIASOUP_CONNECTSTATE_FAIL:
		errorDescription = Str("Output.MediasoupTransport.Error");
		break;
	...
	//add by huchenzhi
	SetLiveBtnText(STR_START_LIVE, true);
	if (m_bStartStream) {
		m_bStartStream = false;
		ShowEndLiveInfo();
	}
...
void OBSBasic::StartRecording()
{
	...
	if (disableOutputsRef)
		return;
	//add by huchenzhi
	DataConfig::GetInstance()->SetRecord(true);
...
{
	if(m_bStreamOperate){
		blog(LOG_INFO,"m_bStreamOperate true");
		return;
	}
	StartStreamTimer();
...
void OBSBasic::PermissionStream() {
	bool confirm = config_get_bool(GetGlobalConfig(), "BasicWindow",
		...
		char roomId[32]={0};
		sprintf(roomId, "%d",
			DataConfig::GetInstance()->GetBeginRsp().roomId);
		char userId[32] = {0};
		sprintf(userId, "%d",
			DataConfig::GetInstance()->GetBeginRsp().userId);
		obs_data_set_string(settings, "room", roomId);
		obs_data_set_string(settings, "userid", userId);
...
void OBSBasic::on_recordButton_clicked()
{
	if (!m_bStartStream) {
		MsgBox::information(this,
			StringConvert::GetInstance()->GetString(STR_START_LIVE_TIP),
			StringConvert::GetInstance()->GetString(
				STR_PLEASE_START_LIVE_TIP));
		return;
	}
	if (outputHandler->RecordingActive()) {
		bool confirm = config_get_bool(GetGlobalConfig(), "BasicWindow",
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "yl-custom-service");
	if (ret <= 0)
		return;
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	QString strDisplay = QT_UTF8(profile);
	int ret;
	...
	ret = GetConfigPath(profiles_path, 512, "yl-custom-service/basic/profiles");
	if (ret <= 0)
		return ret;
...
void OBSBasic::SystemTrayInit()
{
	...
	trayIcon = new QSystemTrayIcon(icon,
		this);
	trayIcon->setToolTip(StringConvert::GetInstance()->GetString(STR_PROCESS));
...
const char *OBSBasic::GetCurrentOutputPath()
{
	...
	if (strcmp(mode, "Advanced") == 0) {
		...
		if (DataConfig::GetInstance()->GetRecord()) {
			std::string strPath =
				DataConfig::GetInstance()->ReadRecordPath();
			if (strPath.empty()) {
...
static void AddSourceLater(void *_data, obs_scene_t *scene, obs_sceneitem_crop *crop)
{
	AddSourceData *data = (AddSourceData *)_data;
	obs_sceneitem_t *sceneitem;

	sceneitem = obs_scene_add_later(scene, data->source, crop);
	obs_sceneitem_set_visible(sceneitem, data->visible);
}

void AddSecret()
{
	const char *secret_name = DataConfig::GetInstance()->GetSecretName();
	std::string strName;
	if (!secret_name || !*secret_name){
		qDebug() << "none value";
		strName = StringConvert::GetInstance()->GetString(STR_SECRET).toStdString();
		DataConfig::GetInstance()->SetScretName(strName.c_str());
	}
	else {
		strName = secret_name;
	}
	...
	if (success&&source) {
		if (main->CreateSecretWindow(source)) {
			//move to top
			obs_sceneitem_t *item =
				obs_scene_find_source(scene, name);
			if (item) {
				obs_sceneitem_set_order(item,
							OBS_ORDER_MOVE_TOP);
...
static void AddScreenShot(int left, int top, int right, int bottom) {
	OBSBasic *main = reinterpret_cast<OBSBasic *>(App()->GetMainWindow());
	OBSScene scene = main->GetCurrentScene();
	if (!scene)
		return;

	const char *screen_name = DataConfig::GetInstance()->GetScreenShotName();
	std::string strName;
	if (!screen_name || !*screen_name){
		qDebug() << "none value";
		strName = StringConvert::GetInstance()->GetString(STR_SCREEN_SHOT).toStdString();
		DataConfig::GetInstance()->SetScreenShotName(strName.c_str());
	}
	else {
		strName = screen_name;
	}
	std::string placeHolderText = strName;
	obs_source_t *source = nullptr;
	int i = 2;
	while ((source = obs_get_source_by_name(strName.data()))) {
		obs_source_release(source);
		char array[512] = "";
	...
	if (!source) {
		...
		if (source) {
			...
			obs_enter_graphics();
			obs_scene_atomic_update_later(scene, AddSourceLater, &data, &crop);
			obs_leave_graphics();
	...
	//				if (count > 0) {
	//					obs_data_set_string(setting, name, obs_property_list_item_string(property, 0));
	//				}
	//			}
	//		}
...
void OBSBasic::OnOpenSecret() {
	AddSecret();
}

void OBSBasic::OnCloseSecret() {
	const char *secret_name = DataConfig::GetInstance()->GetSecretName();
	std::string strName;
	if (!secret_name || !*secret_name) {
		return;
	...
	OBSScene scene = GetCurrentScene();
	obs_source_t *source = obs_get_source_by_name(secret_name);
	obs_sceneitem_t *item = obs_scene_find_source(scene, secret_name);
	if (source && item) {
		if (item) {
			if (source)
				obs_sceneitem_remove_left_widget(
...
void OBSBasic::OnReadStandardOutput() {
	QRect rect(0,0,0,0);
	ParseScreenShot(m_pProcess->readAllStandardOutput(), rect);

	QDesktopWidget* desktopWidget = QApplication::desktop();
	QRect screenRect = desktopWidget->screenGeometry();
	int left = rect.left();
	int top = rect.top() < SCREENSHOT_TOPHEIGHT ? SCREENSHOT_TOPHEIGHT : rect.top();
	int right = screenRect.width() - rect.right();
	int bottom = screenRect.height() - rect.bottom();
	AddScreenShot(left, top, right, bottom);
}

void OBSBasic::SetOriginCtrlsVisible(bool visible) {
	visible = true;
	ui->menubar->setVisible(visible);
	ui->statusbar->setVisible(visible);
	ui->contextContainer->setVisible(visible);
	visible = true;
	ui->scenesDock->setVisible(visible);
	ui->sourcesDock->setVisible(visible);
	ui->mixerDock->setVisible(visible);
	ui->transitionsDock->setVisible(visible);
	ui->controlsDock->setVisible(visible);
	//ui->scenesDock->setFixedHeight(0);
	//ui->sourcesDock->setFixedHeight(0);
	//ui->mixerDock->setFixedHeight(0);
	//ui->transitionsDock->setFixedHeight(0);
	//ui->controlsDock->setFixedHeight(0);
}

void OBSBasic::InitCustomCtrls() {
	if (m_pTopWidget == nullptr) {
		auto movewindow = [&](QPoint pt) {
			QPoint cur = this->geometry().topLeft() + pt;
			this->move(cur);
		};

		m_pTopWidget = new TopWidget(DataConfig::GetInstance()->GetLoginInfo(), ui->topWidget);
		m_pTopWidget->SetMoveCallback(movewindow);
		ui->toplayout->addWidget(m_pTopWidget);
		connect(m_pTopWidget, &TopWidget::ButtonGroupSignals, this, &OBSBasic::OnSysButtonClicked);
		connect(m_pTopWidget, &TopWidget::CloseWindow, this, &OBSBasic::OnCloseWindow);
		connect(m_pTopWidget, &TopWidget::UpdateLoginInfo, this, &OBSBasic::OnUpdateLoginInfo);
		connect(m_pTopWidget, &TopWidget::ExitWindow, this, &OBSBasic::OnExitWindow, Qt::DirectConnection);
	}
	...
	if (m_pBottomCtrl == nullptr) {
		...
		layoutBtm->setContentsMargins(0,0,0,0);
		m_pBottomCtrl = new BottomCtrls(ui->bottomConfigWidget);
		layoutBtm->addWidget(m_pBottomCtrl);
		ui->bottomConfigWidget->setLayout(layoutBtm);
	}
	
	if (m_pUserInfoWidget == nullptr) {
		m_pUserInfoWidget = new UserInfoWidget(DataConfig::GetInstance()->GetLoginToken(), this);
		auto StreamSlot = [&](const StreamState& state) {
			if (state == STREAM_START) {
				on_streamButton_clicked();
			} else if (state == STREAM_STOP){
				FinishLiveDialog dialog(this);
		...
		connect(m_pUserInfoWidget,&UserInfoWidget::StreamStateChanged,StreamSlot);
		connect(this,&OBSBasic::StreamStateSignal,m_pUserInfoWidget,&UserInfoWidget::StreamStateSlot);
		QVBoxLayout *layout = dynamic_cast<QVBoxLayout *>(ui->UserInfo->layout());
		layout->addWidget(m_pUserInfoWidget);
	}

	InitSlider();
	InitRecordTip();
	//connect(m_pBottomCtrl, &BottomCtrls::recordSignal, this,
	//	&OBSBasic::on_recordButton_clicked);
	//connect(m_pBottomCtrl, &BottomCtrls::secretSignal, [&](bool open) {
	//	if (open) {
	//		QMetaObject::invokeMethod(this, "OnOpenSecret",
	//					  Qt::QueuedConnection);
	//	}
	//	else {
	//		QMetaObject::invokeMethod(this, "OnCloseSecret",
	//					  Qt::QueuedConnection);
	//	}
	//});
	auto button_slot = [&](int type) {
		if (type == (int)TYPE_MASK) {
			auto mask_path = [&](std::string &path1, std::string &path2) {
				...
				if (camera_name && *camera_name) {
					...
					if (source) {
						std::string face_mask_name = obs_source_get_display_name("old_face_mask_filter");
						obs_source_t *face_mask_filter = obs_source_get_filter_by_name(source, face_mask_name.c_str());

						if (strcmp(path1.c_str(), ":mask/images/mask/mosaic.png") == 0) {
							if (face_mask_filter)
								obs_source_filter_remove(source,face_mask_filter);
							obs_source_set_mosaic(source, true);
						} else {
							if (strlen(path1.c_str()) == 0 && strlen(path2.c_str()) == 0) {
								if (face_mask_filter)
									obs_source_filter_remove(source,face_mask_filter);
							} else {
								if (!face_mask_filter) {
									face_mask_filter =obs_source_create("old_face_mask_filter",face_mask_name.c_str(),nullptr,nullptr);
									obs_source_filter_add(source, face_mask_filter);
								}
								if (face_mask_filter) {
									obs_data_t *setting = obs_source_get_settings(face_mask_filter);
									obs_data_set_string(setting, "erduo_directory", path1.c_str());
									obs_data_set_string(setting, "huzi_directory", path2.c_str());
									obs_source_update(face_mask_filter, setting);
									obs_source_release(face_mask_filter);
								}
							}
			...
			//EffectDialog dialog(DataConfig::GetInstance()->GetLoginToken(), TYPE_MASK, this);
			//connect(&dialog, &EffectDialog::ChangeFilterPath, this, mask_path, Qt::UniqueConnection);
			MaskDialog dialog(DataConfig::GetInstance()->GetLoginToken(), this);
			connect(&dialog, &MaskDialog::ChangeMaskPath, this, mask_path, Qt::UniqueConnection);
			dialog.exec();
			dialog.close();
		} else if (type == (int)TYPE_BKGROUND) {
			auto bk_path = [&](int type, std::string &path) {
				const char *camera_name = DataConfig::GetInstance()->GetCameraName();
				if (camera_name && *camera_name) {
					obs_source_t *source = obs_get_source_by_name(camera_name);
					if (source) {
						if (path.empty()) {
							...
							/*char img_path[512];
							sprintf(img_path, "%s", path.c_str());*/
							obs_source_set_image_path(source, path.c_str());
							obs_source_set_take_effect(source, true);
							obs_source_set_change_path(source, true);
						}
					}
					obs_source_release(source);
				}
			...
			EffectDialog dialog(DataConfig::GetInstance()->GetLoginToken(), TYPE_BKGROUND, this);
			connect(&dialog, &EffectDialog::ChangeFilterPath, this, bk_path, Qt::UniqueConnection);
			dialog.exec();
			dialog.close();
		} else if (type == (int)TYPE_DEVICE) {
			const char *camera_name = DataConfig::GetInstance()->GetCameraName();
			obs_source_t *camera_source = nullptr;
			if (camera_name && *camera_name) {
				camera_source = obs_get_source_by_name(camera_name);
			}
			obs_source_t *out_put_source = obs_get_output_source(3);

			DeviceDialog dialog(camera_source, out_put_source, this);
			dialog.exec();
		} else if (type == (int)TYPE_QUAILTY) {
			QualityDialog dialog(Config(), this);
			if (dialog.exec() == QDialog::Accepted) {
				std::string out_put = dialog.GetOutputResolut();
				uint32_t newCX;
				uint32_t newCY;
				ConvertResText(out_put.c_str(), newCX, newCY);
				config_set_uint(Config(), "Video", "OutputCX", newCX);
				config_set_uint(Config(), "Video", "OutputCY", newCY);
			}
		} else if (type == (int)TYPE_3DMASK) {
			auto mask_path = [&](int index) {
				const char *camera_name = DataConfig::GetInstance()->GetCameraName();
				if (camera_name && *camera_name) {
					obs_source_t *source = obs_get_source_by_name(camera_name);
					if (source) {
						...
						//} else {
						//	if (!face_mask_filter) {
						//		face_mask_filter =obs_source_create("new_mask_face",face_mask_name.c_str(),nullptr,nullptr);
						//		obs_source_filter_add(source, face_mask_filter);
						//	}
						//}
			...
			MaskDialog3D dialog(m_nMask3dIndex, this);
			connect(&dialog, &MaskDialog3D::ChangeIndex, this, mask_path, Qt::UniqueConnection);
			dialog.exec();
		}
	...
	auto beauty_click = [&] {
		...
		const char *path = DataConfig::GetInstance()->GetMaskPath();
		QMap<BeautyFace_Type, int> mapData;
		const char *value = DataConfig::GetInstance()->GetSkinWhiteValue();
		mapData.insert(BeautyFace_SkinWhite, value ? atoi(value) : 50);
		value = DataConfig::GetInstance()->GetMoultingValue();
		mapData.insert(BeautyFace_Moulting, value ? atoi(value) : 8);
		value = DataConfig::GetInstance()->GetRuddyValue();
		mapData.insert(BeautyFace_Ruddy, value ? atoi(value) : 50);
		value = DataConfig::GetInstance()->GetEyesReshapeValue();
		mapData.insert(BeautyFace_EyesReshape, value ? atoi(value) : 8);
		value = DataConfig::GetInstance()->GetFaceLiftValue();
		mapData.insert(BeautyFace_FaceLift, value ? atoi(value) : 8);
		value = DataConfig::GetInstance()->GetBlusherValue();
		mapData.insert(BeautyFace_Blusher, value ? atoi(value) : 16);
		value = DataConfig::GetInstance()->GetThinChinValue();
		mapData.insert(BeautyFace_ThinChin, value ? atoi(value) : 8);

		LivePartner dialog((Filter_Type)type, mapData, this);
		connect(SignalProxyModel::GetInstance(), &SignalProxyModel::ChangeBeautyFace, this, &OBSBasic::OnBeautyFaceChange, Qt::UniqueConnection);
		connect(SignalProxyModel::GetInstance(), &SignalProxyModel::FilterChange, this, &OBSBasic::OnFilterChange, Qt::UniqueConnection);
		dialog.exec();
	};

	connect(m_pBottomCtrl, &BottomCtrls::buttonSignal, button_slot);
	connect(m_pBottomCtrl, &BottomCtrls::BeautyClicked, beauty_click);

	connect(SignalProxyModel::GetInstance(),
		&SignalProxyModel::AddSourceClicked, this,
		&OBSBasic::OnAddSource);
	connect(SignalProxyModel::GetInstance(),
		&SignalProxyModel::MoveSourceItem, this,
		&OBSBasic::OnMoveSourceItem);
	connect(SignalProxyModel::GetInstance(),
		&SignalProxyModel::SelectSourceItem, this,
		&OBSBasic::OnSelectSourceItem);
	connect(SignalProxyModel::GetInstance(), &SignalProxyModel::EditClicked,
		this, &OBSBasic::OnEditClicked);
	connect(SignalProxyModel::GetInstance(),
		&SignalProxyModel::DeleteClicked, this,
		&OBSBasic::OnDeleteClicked);
}

void OBSBasic::InitSlider()
{
	m_bMicAudioCheck = true;
	if (m_pBottomCtrl!=nullptr) {
		m_pMicSlider = m_pBottomCtrl->GetMicSlider();
		//m_pAudioSlider = m_pBottomCtrl->GetAudioSlider();
	}
}

void OBSBasic::InitMicAudioId() {
	if (m_bMicAudioCheck) {
		const char *deviceId = nullptr;
		obs_fader_t *obs_fader = obs_fader_create(OBS_FADER_CUBIC);
		/*
		*麦克风
		*/
		obs_source_t *source_mic = obs_get_output_source(3);
		if (source_mic) {
			blog(LOG_INFO, "obs_get_output_source mux audio");
			obs_data_t *settings =
				obs_source_get_settings(source_mic);
			if (settings) {
				deviceId = obs_data_get_string(settings,
							       "device_id");
				obs_data_release(settings);
			}

			blog(LOG_INFO,
			     "obs_source_get_settings mic device_id= %s",
			     deviceId);

			if (deviceId) {
				m_strMicId = deviceId;
				//是否可用obs_source_muted(source)
				obs_fader_attach_source(obs_fader, source_mic);
				//m_pMicSlider->blockSignals(true);
				//m_pMicSlider->setValue(
				//	(int)(obs_fader_get_deflection(
				//		      obs_fader) *
				//	      100.0f));
				DataConfig::GetInstance()->SetMicSoundValue(
					(int)(obs_fader_get_deflection(
						      obs_fader) *
					      100.0f));
				//静音
				//m_pMicSlider->setValue(0);
				//m_pMicSlider->blockSignals(false);
			}
			obs_source_release(source_mic);
		}

		//桌面音频
		obs_source_t *source_audio = obs_get_output_source(1);

		if (source_audio) {
			blog(LOG_INFO, "obs_get_output_source desktop audio");
			obs_data_t *settings =
				obs_source_get_settings(source_audio);
			if (settings) {
				deviceId = obs_data_get_string(
					settings, "device_id") /*"default"*/;
				obs_data_release(settings);
			}

			blog(LOG_INFO,
			     "obs_source_get_settings desktop audio device_id= %s",
			     deviceId);

			if (deviceId) {
				m_strAudioId = deviceId;
				//是否可用obs_source_muted(source)

				obs_fader_attach_source(obs_fader,
							source_audio);
		/*		m_pAudioSlider->blockSignals(true);
				m_pAudioSlider->setValue(
					(int)(obs_fader_get_deflection(
						      obs_fader) *
					      100.0f));*/

				//静音
				//m_pAudioSlider->setValue(0);

				DataConfig::GetInstance()->SetAudioSoundValue(
					(int)(obs_fader_get_deflection(
						      obs_fader) *
					      100.0f));
				//m_pAudioSlider->blockSignals(false);
			}
			obs_source_release(source_audio);
		}
		obs_fader_destroy(obs_fader);
		m_bMicAudioCheck = false;
	}
}

void OBSBasic::InitRecordTip() {
	if (m_pRecordSaveTip == nullptr) {
		m_pRecordSaveTip = new RecordSaveTip(this);
		m_pRecordSaveTip->hide();
		auto record_slot = [&](RecordSaveTip::BtnType type) {
			if (type == RecordSaveTip::TYPE_VIEW) {
				const char *filename = config_get_string(
					App()->GlobalConfig(), "RecordSet",
					"LastRecordFileName");
				qDebug() << "filename:" << filename;
				
				QFileDialog::getOpenFileName(
					this, StringConvert::GetInstance()->GetString(STR_OPEN_EXIST_FILE_NAME),
					QString::fromUtf8(filename));
			} else if (type == RecordSaveTip::TYPE_SET) {
		...
		connect(m_pRecordSaveTip, &RecordSaveTip::btnSignal,
			record_slot);
	}
}

void OBSBasic::SetRtmp(const char *url, const char *key) {
	const char *type;
	...
	OBSData url_setting = obs_data_create();
	obs_data_release(url_setting);
	obs_data_set_string(url_setting, "server", url);
	obs_data_set_string(url_setting, "key", key);
...
void OBSBasic::StretchToScreen(OBSSceneItem item) {
	obs_scene_enum_items_select_left_widget_item(GetCurrentScene(), false, select_leftwidget_item);
	//obs_sceneitem_select(item, true);
	obs_sceneitem_select_left_widget(item, true, select_leftwidget_item);
	obs_bounds_type boundsType = OBS_BOUNDS_STRETCH;
	CenterAlignSelectedItems(GetCurrentScene(), item, &boundsType);
	//obs_sceneitem_select(item, false);
	obs_sceneitem_select_left_widget(item, false, select_leftwidget_item);
}

void OBSBasic::RemoveLeftWidgetItem(OBSSceneItem item) {
	if (m_pLeftWidget) {
		m_pLeftWidget->DeleteSourceItem(item);
	}
	if (m_pBottomCtrl) {
		const char *secret_name =
			DataConfig::GetInstance()->GetSecretName();
		if (!secret_name || !*secret_name) {
			return;
		} else {
			OBSScene scene = GetCurrentScene();
			obs_sceneitem_t *item2 =
				obs_scene_find_source(scene, secret_name);
			if (item.Get() == item2) {
				//m_pBottomCtrl->SetSecretState(true);
			}
		}
	}
}

void OBSBasic::SetLiveBtnText(STRINDEX str, bool enable)
{
	if (m_pBottomCtrl) {
		//m_pBottomCtrl->SetLiveBtnText(str, enable);
	}
}

void OBSBasic::OnDeleteClicked(void *scene)
{
	if (scene) {
...
void OBSBasic::MoveItem(obs_sceneitem_t* item) {
	...
	if (source) {
		int w = obs_source_get_width(source);
		const char* name = obs_source_get_name(source);
		const char* id = obs_source_get_id(source);
		if (name && *name && id && *id) {
			vec2 posMove;
			if (bounds.x == 0 && bounds.y == 0)
				posMove.x = pos.x + w*scale.x;
			else
				posMove.x = pos.x + bounds.x;
			posMove.y = pos.y;
			UserMoveSceneItemEvent* event = new UserMoveSceneItemEvent;
			event->x = posMove.x;
			event->y = posMove.y;
