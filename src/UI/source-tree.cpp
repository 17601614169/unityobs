...
void SourceTreeItem::ExitEditMode(bool save)
{
	...
	if (newName.empty()) {
		MsgBox::information(main, QTStr("NoNameEntered.Title"),
	...
	if (newName == prev_name)
		return;
	DataConfig::GetInstance()->CheckNameChanged(prev_name, newName.c_str());
	/* ----------------------------------------- */
	...
	if (exists) {
		MsgBox::information(main, QTStr("NameExists.Title"),
...
{
	{
		...
		for (int i = 0; i < selectedIdxs.count(); i++) {
			...
			obs_sceneitem_select_left_widget(stm->items[idx], true, select_leftwidget_item);
		}

		for (int i = 0; i < deselectedIdxs.count(); i++) {
			int idx = deselectedIdxs[i].row();
			//obs_sceneitem_select(stm->items[idx], false);
			obs_sceneitem_select_left_widget(stm->items[idx], false, select_leftwidget_item);
		}
	}
