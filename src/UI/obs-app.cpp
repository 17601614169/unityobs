...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "yl-custom-service/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "yl-custom-service/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "yl-custom-service/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "yl-custom-service/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "yl-custom-service/updates") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif

	if (GetConfigPath(path, sizeof(path), "yl-custom-service/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static bool MakeUserProfileDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "yl-custom-service/basic/profiles") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "yl-custom-service/basic/scenes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
bool OBSApp::InitGlobalConfig()
{
	...
	int len = GetConfigPath(path, sizeof(path), "yl-custom-service/global.ini");
	if (len <= 0) {
		return false;
	}
...
bool OBSApp::SetTheme(std::string name, std::string path)
{
	...
	if (path == "") {
		...
		string temp = "yl-custom-service/" + name;
...
static void move_basic_to_profiles(void)
{
	...
	if (GetConfigPath(path, 512, "yl-custom-service/basic") <= 0)
		return;
	if (!os_file_exists(path))
		return;

	/* if the profiles directory doesn't already exist */
	if (GetConfigPath(new_path, 512, "yl-custom-service/basic/profiles") <= 0)
		return;
...
static void move_basic_to_scene_collections(void)
{
	...
	if (GetConfigPath(new_path, 512, "yl-custom-service/basic/scenes") <= 0)
		return;
	if (os_file_exists(new_path))
		return;
...
bool OBSApp::event(QEvent* event) {
	if (event->type() == VedioEnabledEventType) {
		VedioDeviceDisabledEvent *eventVedio = nullptr;
		eventVedio = dynamic_cast<VedioDeviceDisabledEvent *>(event);
		if (eventVedio != nullptr) {
			SignalProxyModel::GetInstance()->SetVedioSource(eventVedio->m_strId, eventVedio->m_strName, eventVedio->m_bEnable);
		}	
	}
...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, "yl-custom-service/logs", lastLogFile);
#ifdef _WIN32
	get_last_log(true, "yl-custom-service/crashes", lastCrashLogFile);
#endif

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << "yl-custom-service/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file(false, "yl-custom-service/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("yl-custom-service/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, "yl-custom-service/profiler_data");

		OBSTranslator translator;
		program.installTranslator(&translator);

		QString strQm = QApplication::instance()->applicationDirPath() +
				"\\qtcustom_zh.qm";
		QTranslator customTranslator;
		bool bLoad = customTranslator.load(strQm);
		program.installTranslator(&customTranslator);
		...
		if (0/*already_running*/) {
			...
			MsgBox::information(nullptr,StringConvert::GetInstance()->GetString(STR_PROCESS),
				StringConvert::GetInstance()->GetString(STR_PROCESS_ALREADY_EXIST));
			blog(LOG_WARNING, "================================");
		...
		auto login_slot = [&, login](int btn,QByteArray & bytes) {
			if (btn == LOGIN_CLICK) {
				STLoginRsp stRsp = ParseLogin(bytes);

				if (stRsp.success) {
					DataConfig::GetInstance()->SetLoginInfo(
						stRsp);
					CustomConfig::SetLoginData("token", stRsp.stService.token);
					DataConfig::GetInstance()->SetMenus(stRsp.vecMenu);
					login->accept();
				} else {
					/*OBSMessageBox::warning(
						NULL,
						QString(StringConvert::GetInstance()->GetString(STR_LOGIN_TITLE)),
						QString::fromStdString(stRsp.appMsg));*/
					MsgBox::information(login, QString(MsgBox::Tips()), QString::fromStdString(stRsp.appMsg),EOK);
					//TipsDialog dialog(QString::fromStdString(stRsp.appMsg), login);
					//dialog.exec();
				}
			} else {
				login->reject();
		...
		QObject::connect(login, &LoginDialog::loginSignal, login_slot);
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	string crashFilePath = "yl-custom-service/crashes";
...
static void move_to_xdg(void)
{
	...
	if (snprintf(old_path, 512, "%s/.yl-custom-service", home) <= 0)
		return;
	...
	if (GetConfigPath(new_path, 512, "yl-custom-service") <= 0)
		return;
...
{
	char path[512];
	int pathlen = GetConfigPath(path, 512, "yl-custom-service/basic/profiles");
...
{
  unity_t = new std::thread([]() {
    std::wstring ws;
    ws += L" -batchmode -force-d3d11-singlethreaded";
    //UnityMain(NULL, NULL, (LPWSTR)ws.c_str(), 1);
  });
#if _DEBUG
	AllocConsole();
