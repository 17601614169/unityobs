				 ...
				 QLabel **label)
{
	...
	subLayout->addWidget(spin);
	QString text = QT_UTF8(obs_property_description(prop));
...
bool OBSPropertiesView::SetPrivacy() {
	bool set = false;
	if (properties.get()) {
		obs_property_t *property =
			obs_properties_first(properties.get());
		bool hasNoProperties = !property;
		...
		while (property) {
			...
			if (strcmp(setting, "file") == 0) {
				QString strFile =
					QApplication::applicationDirPath() + "\\images\\yinsi_bg.png";
				
				obs_data_set_string(settings, setting, strFile.toUtf8());
				if (this->callback && !this->deferUpdate)
...
void WidgetInfo::ControlChanged()
{
	const char *setting = obs_property_name(property);
	obs_property_type type = obs_property_get_type(property);
	const char* file_name = obs_data_get_string(view->settings, "file");
	switch (type) {
	case OBS_PROPERTY_INVALID:
		return;
	case OBS_PROPERTY_BOOL:
