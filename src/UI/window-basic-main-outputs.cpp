...
void BasicOutputHandler::PauseStream(bool pause)
{
	obs_output_stream_pause(streamOutput,pause,"both");
}

void BasicOutputHandler::ResetReconnect(bool reconnect) {
	obs_output_set_reconnect(streamOutput,reconnect);
}
...
bool SimpleOutput::ConfigureRecording(bool updateReplayBuffer)
{
	...
	if (updateReplayBuffer) {
		...
		obs_data_set_string(settings, ffmpegOutput ? "url" : "path",
				    strPath.c_str());
		config_set_string(App()->GlobalConfig(), "RecordSet",
				  "LastRecordFileName", strPath.c_str());
	}
...
struct AdvancedOutput : BasicOutputHandler {
	...
	virtual bool RecordingActive() const override;
	virtual bool ReplayBufferActive() const override;
	virtual void PauseStream(bool pause) override;
	virtual void ResetReconnect(bool reconnect)override;
};
...
void AdvancedOutput::PauseStream(bool pause) {
	if (!StreamingActive())
		return;
	obs_output_stream_pause(streamOutput, pause, "both");
 }

void AdvancedOutput::ResetReconnect(bool reconnect) {
	obs_output_set_reconnect(streamOutput, reconnect);
}
