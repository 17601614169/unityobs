...
void OBSBasicPreview::DoSelect(const vec2 &pos)
{
	...
	if (name && *name) {
		if (stricmp(name, main->m_strCloseName.c_str()) != 0) {
			main->m_curSelectItem = item.Get();
		} else {
			if (main->m_curSelectItem) {
				obs_source_t *curSource = obs_sceneitem_get_source(main->m_curSelectItem);
				obs_sceneitem_set_visible(item, false);
				obs_sceneitem_remove_left_widget(main->m_curSelectItem, remove_leftwidget_item);
				main->m_curSelectItem = nullptr;
				return;
			}
		}
	} else {
...
void OBSBasicPreview::DoCtrlSelect(const vec2 &pos)
{
	...
	obs_sceneitem_select_left_widget(item, !selected, select_leftwidget_item);
}
		...
		if (selectionBox) {
			...
			if (altDown || ctrlDown || shiftDown) {
				for (size_t i = 0; i < selectedItems.size();
					...
					/*obs_sceneitem_select(selectedItems[i],
							     true);*/
					obs_sceneitem_select_left_widget(selectedItems[i], true, select_leftwidget_item);
				}
			}
			...
			for (size_t i = 0; i < hoveredPreviewItems.size();
				...
				/*obs_sceneitem_select(hoveredPreviewItems[i],
						     select);*/
				obs_sceneitem_select_left_widget(hoveredPreviewItems[i], select, select_leftwidget_item);
			}
		}
...
static bool move_items(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	...
	obs_source_t *source = obs_sceneitem_get_source(item);
	const char* name = obs_source_get_name(source);
	if (name && *name && stricmp(name, main->m_strCloseName.c_str()) == 0)
		return true;
	...
	if (selected) {
		...
		vec2 scale;
		obs_sceneitem_get_scale(item, &scale);
		pos.x = pos.x + width * scale.x > ovi.base_width ? ovi.base_width - width * scale.x : pos.x;
		pos.y = pos.y + height * scale.y > ovi.base_height ? ovi.base_height - height * scale.y : pos.y;
...
{
	OBSBasic *main = reinterpret_cast<OBSBasic *>(App()->GetMainWindow());
	obs_source_t *stretchSource = obs_sceneitem_get_source(stretchItem);
	const char* name = obs_source_get_name(stretchSource);
	if (name && *name && stricmp(name, main->m_strCloseName.c_str()) == 0) 
		return;
