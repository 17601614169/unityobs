...
bool obs_output_set_reconnect(const obs_output_t *output,bool reconnect){
	if (output != NULL)
		os_atomic_set_bool(&output->reconnecting, reconnect);
}
...
bool obs_output_stream_pause(obs_output_t *output, bool pause, const char *kind)
{
	bool success;
	...
	if (success) {
		os_atomic_set_bool(&output->paused, pause);
		if (pause&&output->info.pause)
			output->info.pause(output->context.data,kind);
		if (!pause&&output->info.resume)
			output->info.resume(output->context.data,kind);
...
bool obs_output_signal_protocol(obs_output_t *output, const char *method,
				const char *key,const char *value)
{
	if (!obs_output_valid(output, "obs_output_signal_protocol"))
		return false;
	if (output->info.signal_protocol) {
		output->info.signal_protocol(output->context.data,method,key,value);
	}
	return true;
}
...
static bool begin_delayed_capture(obs_output_t *output)
{
	...
	if (reconnecting(output)) {
		...
		blog(LOG_INFO, "reconnecting false");
	} else {
		signal_start(output);
	}
...
static void output_reconnect(struct obs_output *output)
{
	...
	if (!reconnecting(output)) {
		...
		blog(LOG_INFO, "reconnecting true");
