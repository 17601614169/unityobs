...
#include <mutex>
#include <json/json.h>
#include <third_party/configor/include/configor/json.hpp>
#include "unity-socket-interface.h"
#include "BackGroundMatting.h"
#include "hand_interface.h"
cv::Mat cocos_image(720, 1280, CV_8UC4);
ULONG start_ts, current_frame_ts;
std::mutex ts_lock;
static int model_index_ = -2;
			     ...
			     bool private1)
{
	if (!obs_context_data_init(&source->context, OBS_OBJ_TYPE_SOURCE,
				   settings, name, hotkey_data, private1))
		return false;
...
static bool obs_source_init(struct obs_source *source)
{
	...
	source->control =
		(obs_weak_source_t *)bzalloc(sizeof(obs_weak_source_t));
				   ...
				   obs_hotkey_t *key, bool pressed)
{
	...
	struct obs_source *source = (obs_source *)data;
...
{
	audio_action action;
	action.timestamp = os_gettime_ns();
	action.type = AUDIO_ACTION_PTM;
	action.set = pressed;
...
obs_source_create_internal(const char *id, const char *name,
			   ...
			   bool private1, uint32_t last_obs_ver)
{
	struct obs_source *source =
		(obs_source *)bzalloc(sizeof(struct obs_source));
			      ...
			      bool private1)
{
	...
	for (size_t i = filters.num; i > 0; i--) {
		...
		obs_source_t *dst_filter =
			obs_source_duplicate(src_filter, new_name, private1);
...
void obs_source_copy_filters(obs_source_t *dst, obs_source_t *src)
{
	...
	duplicate_filters(dst, src, dst->context.private1);
}
...
void obs_source_destroy(struct obs_source *source)
{
	...
	blog(LOG_DEBUG, "%ssource '%s' destroyed",
	     source->context.private1 ? "private " : "", source->context.name);
...
#if 1
	if (strcmp(source->info.id, "dshow_input") == 0) {
		gs_draw_sprite(tex, GS_FLIP_U, 0, 0);
	} else {
...
BackGroundMatting *rvm = nullptr;
long beau = 0;
int state = 0;
cv::Mat img;

void convert_frame2rgb(struct obs_source_frame *frame, uint8_t *rgb)
{
	int width = frame->width;
	int height = frame->height;
	if (frame->format == VIDEO_FORMAT_YUY2) {

		cv::Mat mat_test(cv::Size(width, height), CV_8UC2,
				 frame->data[0]);
		cv::cvtColor(mat_test, mat_test, cv::COLOR_YUV2RGB_YUY2);
		memcpy(rgb, mat_test.data,
		       width * height * 3 * sizeof(uint8_t));

	} else {
		libyuv::I420ToRGB24(frame->data[0], width, frame->data[2],
				    width / 2, frame->data[1], width / 2, rgb,
...
class OneEuroFilter {
public:
	void init(float t0, float x0, float dx0, float min_cutoff, float beta,
		  float d_cutoff)
	{
	...
	OneEuroFilter() { init(0, 0, 0, 10.f, 0.007f, 1); };
	...
	float __call__(float t_e, float x)
	{
	...
	float update(float x) { return __call__(0.03f, x); }
};
...
void send_msg2(Msg msg)
{
	SendFaceVec((char *)&msg, 3 * sizeof(int));
	SendFaceVec((char *)msg.data, msg.len);
}
int msg_index = 1;



static void mask_filter(cv::Mat &src, const std::vector<float> &mask_point,
			int w, int h)
{
	//for (int i = 0; i < 936; i = i + 2) {
	//	mask_point[i] = mask_point[i] * (double)src.cols;
	//	mask_point[i + 1] = mask_point[i + 1] * (double)src.rows;
	//}
	cv::Mat img, mask_mat;
	src.copyTo(img);
	cv::Mat mask;
	cv::Mat dst;
	mask = cv::Mat::zeros(h, w, CV_8UC3);
	dst = cv::Mat::zeros(h, w, CV_8UC3);
	cv::resize(src, mask_mat, cv::Size(50, 50), 0, 0, 0);
	cv::resize(mask_mat, mask_mat, cv::Size(w, h), 0, 0, 0);
	//cv::imshow("mask_mat",mask_mat);
	std::vector<std::vector<cv::Point2i>> contours;
	std::vector<cv::Point2i> points;
	int point_arr[36] = {10,  338, 297, 332, 284, 251, 389, 356, 454,
			     323, 361, 288, 397, 365, 379, 378, 400, 377,
			     152, 148, 176, 140, 150, 136, 172, 58,  132,
			     93,  234, 127, 162, 21,  54,  103, 67,  109};
	for (int i = 0; i < 36; i++) {
		points.push_back(cv::Point2i(
			mask_point[point_arr[i] * 2] * src.cols,
			mask_point[point_arr[i] * 2 + 1] * src.rows));
	}
	contours.push_back(points);
	cv::fillPoly(img, contours, 0);
	cv::drawContours(mask, contours, 0, cv::Scalar(255, 255, 255), -1);
	mask_mat.copyTo(dst, mask);

	cv::Mat dest_image = img + dst;
	//cv::imshow("dest_image",dest_image);
	dest_image.copyTo(src);

	//libyuv::RGB24ToI420(dest_image.data, w * 3, frame->data[0], w, frame->data[2], w / 2, frame->data[1], w / 2, w, h);
}

cv::Mat background(720, 1280, CV_8UC3);

std::vector<float> point(468 * 2);
std::vector<float> tvec(3);
std::vector<float> rquat(4);

//cv::Mat resizeMat, resizeMatf;
cv::Mat pha(144, 256, CV_32F), pha_erode(144, 256, CV_32F), pha_framesize;
//cv::Mat framef;
//cv::Mat mergeImagef;
//cv::Mat mergeImagebackf;

OneEuroFilter pose_filter[7];

int last_select = -1;

void blend(cv::Mat &foreground, cv::Mat &background, cv::Mat &alpha)
{
	uchar *fptr = reinterpret_cast<uchar *>(foreground.data);
	uchar *bptr = reinterpret_cast<uchar *>(background.data);
	float *aptr = reinterpret_cast<float *>(alpha.data);
	//float *outImagePtr = fptr;

	// Loop over all pixesl ONCE

	for (int i = 0; i < 1280 * 720; i++, aptr++) {
		for (int j = 0; j < 3; j++, fptr++, bptr++) {
			*fptr = (*fptr) * (*aptr) + (*bptr) * (1 - *aptr);
		}
	}
}

void blend2(cv::Mat &foreground, cv::Mat &background, cv::Mat &alpha)
{
	uchar *fptr = reinterpret_cast<uchar *>(foreground.data);
	uchar *bptr = reinterpret_cast<uchar *>(background.data);
	float *aptr = reinterpret_cast<float *>(alpha.data);
	...
	for (row = 0; row < 720 ; ++row) {
		for ( col = 0; col < 1280; ++col) {
			...
			idxa = ai*256 + aj;
			for (k = 0; k < 3; ++k) {
				idx = row * 1280*3 + col * 3 + k;
				
				fptr[idx] = fptr[idx] * (aptr[idxa])+(bptr[idx]) * (1 - aptr[idxa]);
			}
		}
	}
}

//Mat send_frame(720, 1280, CV_8UC3);
void processfp468(Mat &mat_test, obs_source_t *source, ULONG ts, int gesture_index)
{

	//mat_test.copyTo(send_frame);

	SAPI_enable_human_seg(beau, source->take_effect);

	SAPI_face_point478_parallel(beau, mat_test, false);

	SAPI_get_landmarks(beau, point.data());
	SAPI_get_translation_vector(beau, tvec.data());
	SAPI_get_rotation_quaternion(beau, rquat.data());

	if (source->take_effect) {
		SAPI_human_seg(beau, mat_test, pha);

		int s = 1 * 2 + 1;
		cv::Mat structureElement = cv::getStructuringElement(
			cv::MORPH_RECT, cv::Size(s, s),
			cv::Point(-1, -1)); //创建结构元
		cv::erode(pha, pha_erode, structureElement);
		cv::resize(pha_erode, pha_framesize, cv::Size(1280, 720));
		//mat_test.convertTo(framef, CV_32FC3);
		blend(mat_test, background, pha_framesize);


		//blend2(mat_test, background, pha_erode);

		//mat_test.copyTo(send_frame);
		//framef.convertTo(send_frame, CV_8UC3);
	} else {
		//mat_test.copyTo(send_frame);
	}
	...
	for (int i = 0; i < n; i = i + 2) {
		point[i] = point[i] * (double)mat_test.cols;
		//point[i] = point[i];
		point[i + 1] = point[i + 1] * (double)mat_test.rows;
		//cv::circle(mat_test,cv::Point2d(point[i] ,point[i + 1] ),
		//		   3, cv::Scalar(255, 0, 0), -1);
	}
	//cv::imshow("mat_test",mat_test);

	//cv::imshow("gggggg",mat_test);
	if (n == 468 * 2) {

		for (int i = 0; i < source->filters.num; i++) {
			obs_source_t *src_filter = source->filters.array[i];

			//memcpy(src_filter->face_point, point.data(),
			//       sizeof(float) * 468 * 2);
			for (int j = 0; j < 468; j++) {
				src_filter->face_point[2 * j] =
					1280 - point[2 * j];
				src_filter->face_point[2 * j + 1] =
					point[2 * j + 1];
			}

			//src_filter->face_point[361] = rvec(0);
			//src_filter->face_point[323] = rvec(1);
			//src_filter->face_point[454] = rvec(2);

			src_filter->face_point[356] = mat_test.cols;
			src_filter->face_point[389] = mat_test.rows;
			//if (src_filter->face_point[101*2]<0) {
			//	abort();
			//}
			//if (face_point[101*2]<0) {
			//	blog(1,"face_point",face_point[101*2]);
		...
		//CocosReq req;
		//cout << "detection all " << req.start << " cost "
		//<< tm_all.getTimeMilli() << endl;

		//req.tvec[0] = beau->tvecOutput[0];
		//req.tvec[1] = beau->tvecOutput[1];
		//req.tvec[2] = beau->tvecOutput[2];

		//InsertTask(req);

		//cv::Mat recvFrame(cv::Size(1280, 720), CV_8UC4);
		...
		configor::json root;
		//root["x"] = pose_filter[0].update(tvec[0]);
		//root["y"] = pose_filter[1].update(tvec[1]);
		//root["z"] = pose_filter[2].update(tvec[2]);
		root["x"] = (tvec[0]);
		root["y"] = (tvec[1]);
		root["z"] = (tvec[2]);
		...
		// qc=qc.conjugate();
		root["qx"] = pose_filter[3].update(rquat[0]);
		root["qy"] = pose_filter[4].update(rquat[1]);
		root["qz"] = pose_filter[5].update(rquat[2]);
		root["qw"] = pose_filter[6].update(rquat[3]);

		root["gesture"] = gesture_index;
		...
		//root["pxs"] = point;
		//root["z468"] = beau->z468;
		//std::vector<uchar> *rgba_data =
		//	new std::vector<uchar>(1280 * 720 * 3);
		//memcpy(rgba_data->data(), send_frame.data, 1280 * 720 * 3);
		//root["rgba"] = *rgba_data;

		auto json_str = root.dump();
		//char msg_head[] = {'a', 'b', 'c', 'd', 'e'};
		//SendFaceVec(msg_head, 5);

		Msg jsData;
		jsData.head = 88;
		jsData.len = strlen(json_str.c_str());
		jsData.cmd = 1;
		jsData.data = (void *)json_str.c_str();

		//SendFaceVec(jsHead.head, 2+sizeof(size_t));
		//SendFaceVec(json_str.c_str(), strlen(json_str.c_str()));
		send_msg2(jsData);

		Msg pd468;
		pd468.head = 22;
		pd468.len = sizeof(float) * 2 * 468;
		pd468.cmd = 2;

		pd468.data = point.data();
		send_msg2(pd468);

		Msg msg_frame;
		msg_frame.head = 22;
		msg_frame.len = 1280 * 720 * 3;

		msg_frame.cmd = 3;
		if (source->mosaic) {
			mask_filter(mat_test, point, 1280, 720);
		}
		cv::flip(mat_test, mat_test, -1);
		msg_frame.data = mat_test.data;
		send_msg2(msg_frame);

		{
			//传输当前选中模型的下标
			std::lock_guard<std::mutex> lk(model_lock_);
			Msg model_select_cmd;
			model_select_cmd.head = 22;
			model_select_cmd.len = sizeof(int);
			model_select_cmd.cmd = 4;
			std::vector<int> vec;
			vec.push_back(model_index_);
			model_select_cmd.data = vec.data();
			send_msg2(model_select_cmd);
		}
		...
		for (int i = 0; i < source->filters.num; i++) {
			obs_source_t *src_filter = source->filters.array[i];
			memset(src_filter->face_point, n,
			       _countof(src_filter->face_point) *
				       sizeof(float));
		}
	}
}
...
bool cocos_finish = true;
bool obs_finish = false;
//int frame_buffer_size = 1;
//string index_str[] = {"0", "1", "2", "3", "4", "5", "6"};
cv::Mat recvFrame5(cv::Size(1280, 720), CV_8UC4);

//cv::TickMeter tickMeter;

std::mutex cocos_image_locker;

static void RecvUnityData(char *data, int length)
{
	//while (!obs_finish) {
	//	Sleep(1);
	//}
	...
	//vector<uchar> datav(data,data+length);
	//cv::Mat recvFrame = cv::imdecode(datav, (int)cv::IMREAD_ANYCOLOR);
	cv::Mat recvFrame(cv::Size(960, 540), CV_8UC4, data);
	cv::flip(recvFrame, recvFrame, 0);

	//cv::imshow("recvFrame", recvFrame);
	//cv::waitKey(3);

	cocos_image_locker.lock();

	//if (cocos_image == NULL) {
	//	cocos_image = new uint8_t[1920 * 1080 * 4];
	//	memset(cocos_image, 0, sizeof(uint8_t) * 1920 * 1080 * 4);
	//}

	if (data != NULL && length > 0) {
		cv::resize(recvFrame, cocos_image, cv::Size(1280, 720));
		//memset(cocos_image, 0, sizeof(uint8_t) * 1920 * 1080 * 4);
		//cocos_image.setTo(cv::Scalar::all(0));
		//recvFrame.copyTo(cocos_image);
		//memcpy(cocos_image.data, recvFrame.data,
		//       std::min<int>(length, sizeof(uint8_t) * length));
	}

	cocos_image_locker.unlock();
	//std::cout << "recieve img" << std::endl;

	//ts_lock.unlock();
	//cocos_finish = true;
}

cv::Mat frame_mat(cv::Size(1280, 720), CV_8UC3);
HandInterface* hand_interface;

static void video_conversion(struct obs_source_frame *frame,
			     obs_source_t *source)
{
	...
	if (strcmp("mediasoup_source", source->info.id) == 0)
		return;

	convert_frame2rgb(frame, frame_mat.data);

	//cv::Mat mat_test(cv::Size(width, height), CV_8UC3, rgb);

	if (beau == 0) {
		hand_interface = factory_function();
		hand_interface->setDraw(false);
		beau = SAPI_init();
		SAPI_set_face_function_shink_size(beau, 1280, 720);
		SAPI_enable_human_seg(beau, false);
		//rgbQueue.init(frame_buffer_size, width, height);
		//rvm = new BackGroundMatting(); // 16 threads
		//rvm->init(6);

		//netModule.init(true);
		//pool.init();

		/*img = cv::imread("D://123.jpg");
			cv::resize(img, img, cv::Size(width, height));*/
		InitUnitySocket(RecvUnityData, "127.0.0.1", 1755);
	}

	//auto res = pool.submit([&] {

	//frame_mat.copyTo(imgll);
	//cv::flip(imgll, imgll, 1);
	//cv::flip(frame_mat, frame_mat, 1);
	//tickMeter.reset();
	//tickMeter.start();
	state =  hand_interface->run(move(frame_mat));
	processfp468(frame_mat, source, 0,state);
	//hand(frame_mat, source, 0);

	cocos_image_locker.lock();

	cocos_image.copyTo(recvFrame5);
	//cv::imshow("recvFrame5", recvFrame5);
	cocos_image_locker.unlock();

	//tickMeter.stop();
	//cout << "face process cost ms "
	//     << tickMeter.getTimeMilli() << endl;

	//});

	//res.get();

	//if(rgbQueue.w!=width && rgbQueue.h != height){
	//	rgbQueue.Reset(4, width, height);
	//	return;
	//}

	//current_frame_ts = rgbQueue.headts();

	//ts_lock.lock();

	//libyuv::RGB24ToI420(rgbQueue.head()->data, width * 3,
	//		    frame->data[0], width, frame->data[1], width / 2,
	//		    frame->data[2], width / 2, width, height);
...
static void background_mask(struct obs_source_frame *frame,
			    obs_source_t *source)
{

	if (source->change_path) {
		background = cv::imread(source->image_path);
		cv::resize(background, background,
			   cv::Size(frame->width, frame->height));
		cv::cvtColor(background, background, cv::COLOR_BGR2RGB);
		//background.convertTo(backgroundf, CV_32FC3);
		source->change_path = false;
	}
}
...
static void obs_source_update_async_video(obs_source_t *source)
{
	if (!source->async_rendered) {
		...
		if (frame) {
			...
			if (source->take_effect && source->image_path) {
				background_mask(frame, source);
			}
			video_conversion(frame, source);
		}
...
static inline void obs_source_main_render(obs_source_t *source)
{
	...
	if (default_effect)
		...
		/*filter中传入facepoint*/
		if (source->info.type == OBS_SOURCE_TYPE_FILTER &&
		    source->info.face_config) {

			source->info.video_face_render(
				source->context.data, source->face_point,
				custom_draw ? NULL : gs_get_effect(),
				recvFrame5.data, start_ts, current_frame_ts);

		} else {
			source->info.video_render(
				source->context.data,
				custom_draw ? NULL : gs_get_effect());
		}
	}
...
void obs_source_save_image(int type, int width, int height, int len,
			   uint8_t *rgba, ULONG start)
{
}
			    ...
			    uint32_t frames, uint64_t ts)
{
	...
	for (size_t i = 0; i < planes; i++) {
		...
		if (resize) {
			bfree(source->audio_data.data[i]);
			source->audio_data.data[i] = (uint8_t *)bmalloc(size);
		}
				     ...
				     const char *tech_name)
{
	...
	if (strcmp(tech_name, "Draw1") == 0) {
		gs_technique_begin_pass(tech, 0);
		gs_eparam_t *param = gs_effect_get_param_by_name(effect, "pp");
		struct matrix4 *pp = (struct matrix4 *)gs_effect_get_val(param);
		...
		//gs_draw_sprite(tex, 0, 200, 200);
		gs_draw_triangle(tex, 0, width, height, v[0], v[1], v[2], v[3],
				 v[4], v[5], v[6], v[7]);
		gs_technique_end_pass(tech);

	} else if (strcmp(tech_name, "Draw2") == 0) {
		gs_technique_begin_pass(tech, 0);
		gs_eparam_t *param = gs_effect_get_param_by_name(effect, "pp1");
		struct matrix4 *pp1 =
			(struct matrix4 *)gs_effect_get_val(param);
		float v1[] = {0, 0, 0, 0, 0, 0, 0, 0};
		v1[0] = pp1->x.x;
		v1[1] = pp1->x.y;
		v1[2] = pp1->y.x;
		v1[3] = pp1->y.y;
		...
		//gs_technique_begin_pass(tech, 1);
		//gs_draw_sprite(tex, 0, 200, 200);
		gs_draw_triangle(tex, 0, width, height, v1[0], v1[1], v1[2],
...
{
	struct source_enum_data *data = (source_enum_data *)param;
...
static void check_descendant(obs_source_t *parent, obs_source_t *child,
			     void *param)
{
	struct descendant_info *info = (descendant_info *)param;
...
static void apply_audio_actions(obs_source_t *source, size_t channels,
				size_t sample_rate)
{
	float *vol_data = (float *)malloc(sizeof(float) * AUDIO_OUTPUT_FRAMES);
...
void obs_source_send_model_index(int index)
{
	std::lock_guard<std::mutex> lk(model_lock_);
	model_index_ = index;
}
