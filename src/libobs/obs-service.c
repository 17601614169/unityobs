						  ...
						  bool private)
{
	...
	//add by huchenzhi
	const char *service_ = service->info.get_url(service->context.data);
	const char *key_ = service->info.get_key(service->context.data);
...
const char *obs_service_get_userid(const obs_service_t *service)
{
	...
	if (!service->info.get_userid)
		return NULL;
	return service->info.get_userid(service->context.data);
}
