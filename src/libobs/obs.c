...
void obs_enum_scenes(bool (*enum_proc)(void *, obs_source_t *), void *param)
{
	...
	while (source) {
		...
		if (source->info.type == OBS_SOURCE_TYPE_SCENE &&
		    !source->context.private1 && !enum_proc(param, source)) {
			break;
		}
					...
					void *(*addref)(void *))
{
	...
	context = *first;
	while (context) {
		if (!context->private1 && strcmp(context->name, name) == 0) {
...
const char *obs_source_get_image_path(obs_source_t *source)
{
	if (source)
...
void obs_source_set_image_path(obs_source_t *source, const char *path)
{
	if (source->image_path && path)
		strcpy_s(source->image_path, 512, path);
}

bool obs_source_get_take_effect(obs_source_t *source)
{
	return source->take_effect;
}

void obs_source_set_take_effect(obs_source_t *source,bool effect)
{
	source->take_effect = effect;
}

bool obs_source_get_change_path(obs_source_t *source)
{
	return source->change_path;
}

void obs_source_set_change_path(obs_source_t *source,bool change)
{
	source->change_path = change;
}

bool obs_source_get_mosaic(obs_source_t *source) {
	return source->mosaic;
}

void obs_source_set_mosaic(obs_source_t *source,bool mosaic) {
...
static obs_source_t *obs_load_source_type(obs_data_t *source_data)
{
	...
	const char *image_path = obs_data_get_string(source_data, "image_path");
	obs_source_set_image_path(source, image_path);

	take_effect = obs_data_get_bool(source_data, "take_effect");
	obs_source_set_take_effect(source, take_effect);

	change_path = obs_data_get_bool(source_data, "change_path");
	obs_source_set_change_path(source, change_path);

	mosaic = obs_data_get_bool(source_data, "mosaic");
	obs_source_set_mosaic(source, mosaic);
...
obs_data_t *obs_save_source(obs_source_t *source)
{
	...
	const char *image_path = obs_source_get_image_path(source);
	bool take_effect = obs_source_get_take_effect(source);
	bool change_path = obs_source_get_change_path(source);
	bool mosaic = obs_source_get_mosaic(source);
	...
	obs_data_set_string(source_data, "image_path", image_path);
	obs_data_set_bool(source_data, "take_effect", take_effect);
	obs_data_set_bool(source_data, "change_path", change_path);
	obs_data_set_bool(source_data, "mosaic", mosaic);
					    ...
					    void *data_)
{
	...
	while (source) {
		if ((source->info.type != OBS_SOURCE_TYPE_FILTER) != 0 &&
		    !source->context.private1 && !source->removed &&
		    cb(data_, source)) {
			      ...
			      const char *name)
{
	...
	context->name = dup_name(name, context->private1);
