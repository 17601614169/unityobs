				       ...
				       bool duplicate_private_data)
{
	...
	if (duplicate_hotkeys && !dst_scene->source->context.private1) {
...
obs_sceneitem_t *obs_scene_select_find_source_not_match(obs_scene_t *scene,
							const char *id)
{
	struct obs_scene_item *item;
	...
	item = scene->first_item;
	while (item) {
		if (stricmp(item->source->info.id, id) != 0)
			break;
...
void obs_scene_enum_items_select(obs_scene_t *scene,bool selected) {
	struct obs_scene_item *item;
...
void obs_scene_enum_items_select_left_widget_item(obs_scene_t *scene, bool selected, select_left_widget_item_func func)
{
	struct obs_scene_item *item;
	...
	while (item) {
		...
		obs_sceneitem_addref(item);
		obs_sceneitem_select_left_widget(item, selected, func);
		obs_sceneitem_release(item);
					       ...
					       obs_sceneitem_t *insert_after)
{
	...
	if (!scene->source->context.private1)
...
void obs_sceneitem_select_left_widget(obs_sceneitem_t *item, bool select,
				      select_left_widget_item_func func)
{
	struct calldata params;
	uint8_t stack[128];
	const char *command = select ? "item_select" : "item_deselect";
...
obs_sceneitem_t *obs_scene_add_later(obs_scene_t *scene, obs_source_t *source,
				     struct obs_sceneitem_crop *crop)
{
	obs_sceneitem_t *item = obs_scene_add_internal(scene, source, NULL);
...
void obs_scene_atomic_update_later(obs_scene_t *scene,
				   obs_scene_atomic_update_later_func func,
				   void *data, struct obs_sceneitem_crop *crop)
{
	if (!scene)
		return;
