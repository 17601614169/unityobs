...
static void build_sprite_sticker(struct gs_vb_data *data, float s_w, float s_h,
			 float start_u, float end_u, float start_v, float end_v,uint32_t center_x,
	uint32_t center_y)
{
	struct vec2 *tvarray = data->tvarray[0].array;


	float start_pointx = center_x - s_w/2 >0 ? center_x - s_w/2 :0;
	float start_pointy = center_y - s_h / 2 > 0 ? center_y - s_h / 2 : 0;

	vec3_set(data->points, start_pointx, start_pointy, 0.0f);
	vec3_set(data->points + 1, start_pointx  + s_w, start_pointy + 0.0f,
		 0.0f);
	vec3_set(data->points + 2, start_pointx + 0.0f, start_pointy + s_h,
		 0.0f);
	vec3_set(data->points + 3, start_pointx + s_w, start_pointy + s_h,
		 0.0f);

	vec2_set(tvarray + 0, start_u, start_v);
	vec2_set(tvarray + 1, end_u, start_v);
	vec2_set(tvarray + 2, start_u, end_v);
	vec2_set(tvarray + 3, end_u, end_v);
}
...
static inline void build_sprite_norm_staicker(struct gs_vb_data *data, float fcx, float fcy,
					      uint32_t flip, uint32_t center_x,
					      uint32_t center_y)
{
	float start_u, end_u;
	float start_v, end_v;
	...
	assign_sprite_uv(&start_u, &end_u, (flip & GS_FLIP_U) != 0);
	assign_sprite_uv(&start_v, &end_v, (flip & GS_FLIP_V) != 0);
	build_sprite_sticker(data, fcx, fcy, start_u, end_u, start_v, end_v,center_x,center_y);
}
...
void gs_draw_triangle(gs_texture_t *tex,  uint32_t flip, uint32_t width,
          uint32_t height,          
          ...
          float v6, float v7, float v8)
{
	  ...
	  float start_u, end_u;
	  float start_v, end_v;
	  float width1 = (float)gs_texture_get_width(tex);
	  float height1 = (float)gs_texture_get_height(tex);

	  assign_sprite_rect(&start_u, &end_u, width1, false);
	  assign_sprite_rect(&start_v, &end_v, height1, false);

	  struct vec2 *tvarray = data->tvarray[0].array;

	  //vec3_set(data->points , 100, 100.f, 0);
	  //vec3_set(data->points + 1, 200.0, 50.0f, 0.0f);
	  //vec3_set(data->points + 2, 150.0f, 600.0f, 0.0f);
	  //vec3_set(data->points + 3, 600.0, 400.0, 0.0f);

	  //vec3_set(data->points+1, 100, 100.f, 0);
	  //vec3_set(data->points + 3, 200.0, 100.0f, 0.0f);
	  //vec3_set(data->points + 0, 100.0f, 200.0f, 0.0f);
	  //vec3_set(data->points + 2, 300.0, 300.0, 0.0f);
   
	  vec3_set(data->points , v1, v2, 0);
	  vec3_set(data->points + 1, v3, v4, 0.0f);
	  vec3_set(data->points + 2, v5, v6, 0.0f);
	  vec3_set(data->points + 3, v7, v8, 0.0f);

	  vec2_set(tvarray, 0, 0);
	  vec2_set(tvarray + 1, 1, 0);
	  vec2_set(tvarray + 2, 0, 1);
	  vec2_set(tvarray + 3, 1, 1);

	  gs_vertexbuffer_flush(graphics->sprite_buffer);
	  gs_load_vertexbuffer(graphics->sprite_buffer);
	  gs_load_indexbuffer(NULL);
	  //gs_set_3d_mode(100,50,1000);
	  /*gs_perspective(30, 1, 100, 1000);*/
	  gs_draw(GS_TRISTRIP, 0, 0);
}
