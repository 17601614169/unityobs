...
static LONG CALLBACK exception_handler(PEXCEPTION_POINTERS exception)
{
	...
	snprintf(file, sizeof(file), "%d-%02d-%02d%c%02d-%02d-%02d.%s",
		 ...
		 cur_time->tm_mday, true ? '-' : ' ', cur_time->tm_hour,
		 cur_time->tm_min, cur_time->tm_sec, "dmp");
	...
	einfo.ExceptionPointers = exception;
	einfo.ClientPointers = FALSE;

	MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile,
			  MiniDumpNormal | MiniDumpWithDataSegs, &einfo, NULL,
			  NULL);
	CloseHandle(hFile);
...
void initialize_crash_handler(void)
{
	...
	if (!initialized) {
		if (SetUnhandledExceptionFilter((LPTOP_LEVEL_EXCEPTION_FILTER)exception_handler) ==NULL) {
			initialized = false;
		} else {
			initialized = true;
		}
