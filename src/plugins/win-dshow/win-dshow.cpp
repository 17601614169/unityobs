...
void DShowInput::DShowLoop()
{
	while (true) {
		...
		case Action::ActivateBlock: {
			...
			const char *value = obs_data_get_string(settings, "video_device_id");
			if (!Activate(settings)) {
				obs_source_output_video2(source, nullptr);
				VedioDeviceNotEnabled(id, value, false);
			} else {
				VedioDeviceNotEnabled(id, value, true);
			}
			if (block)
...
#if MIRROR_ENABLED
		unsigned char temp;
		for (int j = 0; j < cy_abs; j++) {
			for (int i = 0; i < cx; i=i+4){
				for (int n = 0;n < 4; n++){
					temp = data[j * cx * 2 + i + n];
					data[j * cx * 2 + i + n] =
					data[j * cx * 2 + (cx * 2 - 1) - i - (3 - n)];
					data[j * cx * 2 + cx * 2 - 1 - i -(3 - n)] = temp;
				}
			}
		}
