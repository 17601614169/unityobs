			 ...
			 struct obs_source_frame2 *frame, bool *got_output)
{
	...
	*got_output = false;
	...
	for (size_t i = 0; i < MAX_AV_PLANES; i++) {
#if MIRROR_ENABLED
		int width = decode->frame->width;
		int height = decode->frame->height;
		int linecount = decode->frame->linesize[i];
		uint8_t *temp_data = decode->frame->data[i];
		height = linecount > 0 ? height / (width / linecount):0;
		for (int a = 0; a < height; a++) {
		for (int b = 0; b < linecount / 2; b++) {
		uint8_t temp = temp_data[a * linecount + b];
		temp_data[a * linecount + b] = temp_data[a * linecount + linecount - 1 - b];
		temp_data[a * linecount + linecount - 1 - b] = temp;
		}
		}

		frame->data[i] = temp_data;
#else
