...
#pragma warning(disable : 4819 4101)
...
namespace mediasoupclient
{
	  ...
	  const json& appData)
	{
		...
		try
		{
			...
			// May throw.
			this->listener->OnProduce(this, producerListener, sendResult, track, appData);
		}
		catch (MediaSoupClientError& error)
		{
			this->sendHandler->StopSending(sendResult.localId);
	...
	Producer* SendTransport::CreateProducer(
	  Producer::Listener* producerListener,
	  const std::string& producerId,
	  const SendHandler::SendResult& sendResult,
	  webrtc::MediaStreamTrackInterface* track,
	  const json& appData)
	{
		auto* producer = new Producer(
