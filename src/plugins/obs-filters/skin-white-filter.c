...
static const uint32_t LUT_WIDTH = 16;
...
static const char *color_grade_filter_get_name(void *unused)
{
	...
	return obs_module_text("SkinWhiteFilter");
}
...
static void color_grade_filter_update(void *data, obs_data_t *settings)
{
	...
	char *effect_path = obs_module_file("skin_white_filter.effect");
...
static obs_properties_t *color_grade_filter_properties(void *data)
{
	...
	if (s && s->file && *s->file) {
		...
		char *lut_dir = obs_module_file("D:\\obsproj\\studio\\build\\rundir\\Debug\\data\\obs-plugins\\obs-filters\\LUTs\\skin_lookup.png");
...
struct obs_source_info skin_white_filter = {
