...
Q_SIGNALS:
    ...
    void queryAnchor(const QString & queryId);
    private:
    explicit WebJsObject(QObject *parent = 0);
};

class QTCUSTOM_EXPORT WebEngineView : public QWebEngineView {
    Q_OBJECT
public:
...
class QTCUSTOM_EXPORT WebEngineWidget : public BaseWebWidget {
    Q_OBJECT
...
public:
    ...
    virtual void SetContent(const QByteArray& data, const QString& mimeType = QString(), const QUrl& baseUrl = QUrl()) override final;
    virtual void SetHtml(const QByteArray& data,const QUrl& baseUrl = QUrl())override final;
...
public:
    ...
    void ResetView() ;
    void ShowScrollBar(bool is_show);
...
*/
class QTCUSTOM_EXPORT WebBrowserWidget : public WebEngineWidget {
    Q_OBJECT
public:
